...
wait_for_db() {
    ...
    while ! pg_isready -h "$db_host" -p "$db_port" -U "$CHAT_POSTGRES_USER"; do
        sleep 2
    done
...
wait_for_db "chat-db-container" "5434"
...
if [ "$(python manage.py shell -c 'from django.contrib.auth import get_user_model; User = get_user_model(); print(User.objects.filter(is_superuser=True).exists())')" = "False" ]; then
    ...
    python manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser('${CHAT_SUPERUSER_USERNAME}', '${CHAT_SUPERUSER_EMAIL}', '${CHAT_SUPERUSER_PASSWORD}')"
else
