...
def init_user_stats(request, userID):
    ...
    url = f"https://nginx-container/api/game/create_game_stats/{userID}/"

    response = requests.post(url, headers=request_headers, cookies={"access_token": access_token}, verify=False)
    ...
    url = f"https://nginx-container/api/tournaments/create_tournament_stats/{userID}/"
    response = requests.post(url, headers=request_headers, cookies={"access_token": access_token}, verify=False)
    if(response.status_code != 201):
...
def delete_user_stats(request, userID):
    ...
    url = f"https://nginx-container/api/game/delete_game_stats/{userID}/"

    response = requests.delete(url, headers=request_headers, cookies={"access_token": access_token}, verify=False)
   
    if(response.status_code != 204):
        raise Exception(f"Error encountered while deleting game stats {response.text}")
    
    url = f"https://nginx-container/api/tournaments/delete_tournament_stats/{userID}/"
    response = requests.delete(url, headers=request_headers, cookies={"access_token": access_token}, verify=False)
    if(response.status_code != 204):
...
def getProfileStats(request, userID):
    ...
    url = f"https://nginx-container/api/game/profile_stats/{userID}/"
